...
// 세션 내 재조회 시 즉시 표시를 위한 간단한 클라이언트 캐시
...
export default function AirQualityPanel({ sidoCode }: { sidoCode: string }) {
  ...
  const load = useCallback(
    async (code: string, opts?: { skipCache?: boolean }) => {
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div>
          ...
          <p className="text-xs text-slate-500">
            ...
            {airQuality.stationName ? ` · ${airQuality.stationName} 측정소` : ""}
          </p>
        </div>
      ...
      {forecast && <ForecastCards days={forecast.days} />}
